        ...
        async with semaphore:
            ...
            async with client_session.get(url) as response:
                if response.status == 200:
                    ...
                    print(
                        "Error 503 occurred when downloading "
                        + track_id
                    ...
                    print(
                        "Error " + str(response.status) + " occurred "
...
async def download(filt, num_chunks=4, max_songs=1000):
    ...
    if max_songs > recordings_num:
        print(str(recordings_num) + " recordings found, downloading...")
    else:
        print(
            str(recordings_num)
            + " recordings found, downloading "
            + str(max_songs)
            + "..."
        )
    ...
    async with aiohttp.ClientSession() as client_session:
        ...
        tasks = [
            http_client(track_tuple, client_session)
            for track_tuple in url_list[1][: max(max_songs, recordings_num)]
        ]
...
def purge(num):
    print("Removing all audio folders with fewer than " + str(num) + " recordings.")
    path = "dataset/audio/"
...
def gen_meta(path="dataset/audio/"):
    ...
    if not_found:
        print(
            str(len(not_found)) +
            " recordings must have their " " metadata downloaded."
        )
...
def main():
    ...
    if len(sys.argv) == 2:
        print(
            "Commands must be given in a '-command parameter' format. "
